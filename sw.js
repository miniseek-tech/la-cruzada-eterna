const CACHE_NAME='cruzada-eterna-v13';
...
function injectMusic(response){
  ...
  return response.text().then(html=>{
    ...
    if(!html.includes('necrofago-inline.js'))html=html.replace(/<\/body>/i,'<script src="./assets/necrofago-inline.js?v=1"></script></body>');
    if(/bestiario-art\.js(?:\?v=\d+)?/i.test(html)){
      html=html.replace(/bestiario-art\.js(?:\?v=\d+)?/ig,'bestiario-art.js?v=10');
    }else{
      html=html.replace(/<\/body>/i,'<script src="./bestiario-art.js?v=10"></script></body>');
    }
